# Temporal convolutional network for EEG — Gemein et al., 2020.
# Adapted from braindecode (TCN and its helper layers):
...
class TCN(EEGModuleMixin, nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.mean(dim=2)  # average over the time dimension
        out = self.final_layer(x)  # (batch, 2)
        return out
