# Deep4Net / deep ConvNet — Schirrmeister et al., 2017.
# Adapted from braindecode (Deep4Net and its helper layers):
#   https://github.com/braindecode/braindecode
# braindecode is distributed under the BSD-3-Clause licence, whose terms and original
# copyright notice apply to the portions derived from it:
#   https://github.com/braindecode/braindecode/blob/master/LICENSE
import torch
...
class Deep4Net(nn.Module):
    def __init__(
        ...
        self.batch_norm = nn.BatchNorm2d(n_filters_4)  # batch normalization
        ...
        if self.split_first_layer:
            ...
            self.layers.add_module('conv_spat', nn.Conv2d(n_filters_time, n_filters_spat, (1, 1), bias=not self.batch_norm))
            n_filters_conv = n_filters_spat
        else:
            self.layers.add_module('conv_time', nn.Conv2d(n_chans, n_filters_time, (filter_time_length, 1), stride=(self.conv_stride, 1), padding=(filter_time_length // 2, 0), bias=not self.batch_norm))
        ...
        def add_conv_pool_block(n_filters_before, n_filters, filter_length, block_nr):
            ...
            self.layers.add_module('conv' + suffix, nn.Conv2d(n_filters_before, n_filters, (filter_length, 1), stride=(self.conv_stride, 1), padding=(filter_length // 2, 0), bias=not self.batch_norm))
            if self.batch_norm:
    ...
    def forward(self, x):
        ...
        # If the time dimension is 1, remove it as well
        if x.shape[-1] == 1:
            x = x.squeeze(-1)
