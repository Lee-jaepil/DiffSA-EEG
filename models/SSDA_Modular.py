...
# ResidualConvBlock (behaviour depends on whether CBAM is enabled)
...
# SSDA Layers
def _sda_norm(out_channels):
    # env var SDA_NORM=group selects GroupNorm, otherwise BatchNorm (default behaviour)
    import os
    if os.environ.get('SDA_NORM', 'batch').lower() == 'group':
        ng = 8 if out_channels % 8 == 0 else (4 if out_channels % 4 == 0 else 1)
        return nn.GroupNorm(ng, out_channels)
...
class SparseEncoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.bn = _sda_norm(out_channels)
...
class SpatialFilterLayer(nn.Module):
    ...
    def _initialize_weights(self, n_channels, n_components):
        ...
        # keep only the first n_components orthogonal vectors
    ...
    def forward(self, x):
        ...
        # filter normalization: scale each component vector to unit norm
        ...
        if self.training:
            ...
            self.reg_loss = reg_loss  # stored to be added to the total loss later
...
class ModularEncoder(nn.Module):
    def __init__(self, in_channels, dim=64, n_spatial_components=8, 
        ...
        # adjust input channels depending on whether the spatial filter is used
...
class ModularDecoder(nn.Module):
    ...
    def forward(self, x0, encoder_out, diffusion_out):
        ...
        if self.use_spatial_filter:
            ...
            # Just reshape if no spatial filter
            B, C, L = out.shape
            out = out[:, :self.in_channels, :]  # Ensure correct number of channels
...
class ModularDiffE(nn.Module):
    def __init__(self, in_channels, dim=64, n_feat=128, n_classes=2, n_spatial_components=8,
        ...
        # build modular encoder and decoder
